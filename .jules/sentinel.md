...
## 2024-06-12 - Missing Global Security Headers
**Vulnerability:** The application was missing standard HTTP security headers (such as `Strict-Transport-Security`, `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, and `X-XSS-Protection`) in its deployment configuration.
**Learning:** For serverless applications deployed on Vercel, security headers should be configured globally at the infrastructure level via the `headers` block in `vercel.json`, rather than being manually set in every individual edge function response or left to client-side logic.
**Prevention:** Always verify that `vercel.json` includes a `headers` section with standard security controls applied to a wildcard route (e.g., `/(.*)`) during project bootstrapping.
