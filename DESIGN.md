...
- Frosted glass appears only on panels floating over the 3D scene, never as decoration
...
### Secondary
- **Aged Brass** (#B8956A): The single decorative accent. Used on primary CTA buttons (the brass-plate component), active selection rings, and hardware metaphors in the 3D scene.
...
### Tertiary
- **Dusty Sage** (#9BAE94): The one chromatic departure from the warm brown spectrum. Used for the bench cushion in the 3D scene and as a background tint for drawer-type compartment tags. Never used on body text.
...
Elevation is warm and low. Shadows suggest that surfaces sit in a lit room, not on a white table. The 3D walnut cabinet is the hero; 2D surfaces never compete through dramatic depth. Glassmorphism appears only when a panel genuinely floats over the 3D scene — functional, not aesthetic.
...
- **Frosted Panel** (`0 6px 20px -8px rgba(40, 20, 10, 0.25)` + `backdrop-filter: blur(14px) saturate(140%)`): Floating sheets over the 3D scene. The `.frost` component. Purposeful glassmorphism only.
...
A frosted bottom sheet that slides over the 3D scene to show compartment contents.
...
- Never use frost on static layout elements. If it doesn't float over the 3D scene, use a solid surface.
...
- **Do** let the 3D cabinet be the visual hero; keep 2D surfaces quiet.
